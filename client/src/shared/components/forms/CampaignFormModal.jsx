...
export function CampaignFormModal({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await aiService.checkSpamRisk(formData.subject, formData.content);
        if (res.analysis) {
  ...
  const handleGenerateAiSubjects = async () => {
    ...
    try {
      setAiLoading(true);
      const res = await aiService.getSubjectLines(topic, 'Subscribers', 'Engaging');
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            className="w-full h-[40px] px-3.5 bg-[#16191F] border border-[rgba(255,255,255,0.08)] rounded-lg text-sm text-[#F4F5F7] placeholder-[#6B7280] focus:outline-none focus:border-[#E8A33D] transition-colors"
          />
        </div>

        {/* AI Suggestions Drawer */}
        {showAiModal && aiSuggestions.length > 0 && (
          <div className="p-3 bg-[#111317] border border-[#E8A33D]/30 rounded-lg space-y-2 animate-fade-in">
            ...
            <div className="space-y-1.5">
              {aiSuggestions.map((s, idx) => (
                <div
                  ...
                  className="p-2 bg-[#1B1E24] hover:bg-[#232730] border border-[rgba(255,255,255,0.06)] rounded flex items-center justify-between cursor-pointer transition-colors group"
                >
        ...
        {spamAnalysis && (
          <div className="p-3 bg-[#16191F] border border-[rgba(255,255,255,0.06)] rounded-lg flex items-center justify-between text-xs">
            ...
            <span className="text-[11px] text-[#6B7280] truncate max-w-[200px]">
        ...
        <div className="space-y-1.5">
          ...
          <select
            ...
            className="w-full h-[40px] px-3.5 bg-[#16191F] border border-[rgba(255,255,255,0.08)] rounded-lg text-sm text-[#F4F5F7] focus:outline-none focus:border-[#E8A33D] transition-colors"
          >
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            className="w-full p-3 bg-[#16191F] border border-[rgba(255,255,255,0.08)] rounded-lg text-xs font-mono text-[#F4F5F7] placeholder-[#6B7280] focus:outline-none focus:border-[#E8A33D] transition-colors"
          />
