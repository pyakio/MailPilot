...
export function Campaigns() {
  ...
  const handleSave = async (payload) => {
    try {
      ...
      if (editingCampaign) {
        ...
        addToast({ title: 'Campaign Updated', message: `Saved changes to "${payload.name}".`, type: 'success' });
      } else {
        await campaignService.createCampaign(payload);
        addToast({ title: 'Campaign Created', message: `Added new campaign "${payload.name}".`, type: 'success' });
      }
  ...
  const handleSendNow = async (id) => {
    try {
      ...
      addToast({ title: 'Campaign Dispatched!', message: 'Email sequence triggered for delivery.', type: 'success' });
      fetchData();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Design, schedule, and track performance of all automated email broadcasts."
      ...
      <Card noPadding>
        <div className="p-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800">
          <SearchBar value={search} onChange={setSearch} placeholder="Search campaigns by title or subject..." />
