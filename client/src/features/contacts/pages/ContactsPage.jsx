...
export function ContactsPage() {
  ...
  const handleAddContact = async (data) => {
    try {
      ...
      addToast({ title: 'Contact Added', message: `Added "${data.email}".`, type: 'success' });
  ...
  const handleImportCsv = async (rows) => {
    try {
      ...
      addToast({
        ...
        message: `Imported ${res.importedCount} subscribers.`,
        type: 'success',
      });
  ...
  const handleDeleteContact = async () => {
    ...
    try {
      ...
      addToast({ title: 'Contact Deleted', message: 'Subscriber removed.', type: 'info' });
      setDeletingId(null);
      fetchContacts();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Contacts"
        description="Organize subscriber audience lists and import contacts."
      ...
      <Card noPadding>
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description={search ? `No subscriber matches "${search}".` : 'Add subscribers to build your audience.'}
